...
@Component({
  ...
  template: `
    <div class="border-b border-border flex flex-1 flex-col" [attr.data-state]="isOpen() ? 'open' : 'closed'">
      <button
        type="button"
        role="button"
        [id]="'accordion-' + zValue()"
        class="group flex flex-1 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-offset-2 focus-visible:ring-ring font-medium items-center justify-between px-0.5 py-4 text-sm w-full hover:cursor-pointer"
        [class]="class()"
        (click)="toggle()"
        ...
        tabindex="0"
      >
        <span class="group-hover:placeholderClass">
          {{ zTitle() }}
        </span>
